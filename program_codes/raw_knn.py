''' 점 (X, F(x))와 새로운 X값 new_x가 주어졌을 때, new_x와 가장 거리가 가까운 k개의 x좌표 [x_0, x_1, ... , x_k]를 
    찾고 이에 대응하는 [F(x_0), F(x_1), ... , F(x_k)]를 평균해 new_x의 F(new_x)값을 근사하는 코드 ''' 
...
def find_n_nearest_value(X, Y, new_x, k):
    ...
    numbers = [0] * k    # 가장 거리가 가까운 K개의 좌표 저장 
    num_idx = [0] * k    # 해당 값의 index 저장 
    ...
    for data in X:
        ...
        if idx < k:   # idx 값이 k 미만인 경우 numbers 배열에 length 초기값 저장 
            numbers[idx] = length
            num_idx[idx] = idx 
            
            # print(num_idx)
        else:   # idx 값이 k 이상인 경우
            # 최댓값을 찾고 최댓값보다 해당 값이 작을 경우 교체  
            max_value = 0
            tmp_idx = 0   # numbers 배열에서 최댓값의 위치 
            max_idx = 0    
            for val in numbers: 
                if val >= max_value:
                    max_value = val 
            ...
            if numbers[tmp_idx] >= length: 
                numbers[tmp_idx] = length
                num_idx[tmp_idx] = idx 
    
        idx += 1   
    
    # new_x의 F(new_x)값 근사하기 
    pred_y = 0 
    for val in num_idx:
        # print(val, Y[val])
        pred_y += Y[val] 
